...
import sqlite3
from crud_functions import Users
...
async def set_username(message, state):
    global user
    if user.is_included(message.text):
        await state.update_data(username = message.text)
        await message.answer('Введите свой email:')
        await RegistrationState.email.set()
    else:
        await message.answer('Такой пользователь уже существует. Введите имя еще раз', )
...
async def set_age(message, state):
    ...
    user.add_user(data['username'], data['email'], data['age'])
    await state.finish()
